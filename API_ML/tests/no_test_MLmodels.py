...
def mock_load_data(raw_data_path):
    ...
    X = pd.DataFrame({
        'Batch_OGD_Technicien': ['A', 'B'],
        'Batch_OGD_KC8_batch': ['X', 'Y'],
        'Batch_OGD_Analyses': ['M', 'N'],
        'month_production': [1, 2],
        'day_production_start': [1, 2],
        'exfo_gde_maree': ['high', 'low'],
        'Batch_OGD_Temperature': [25.0, 30.0],
        'Batch_OGD_Agitation': [100, 150],
        'Batch_OGD_room_HR': [50, 60],
        'conc_KC8': [0.5, 0.6],
        'Batch_OGD_room_T': [22.0, 23.0],
        'days_exfo': [5, 6]
    })
    y = pd.Series([1.0, 2.0], name='QC_Conc_OGD')
    return X, y
