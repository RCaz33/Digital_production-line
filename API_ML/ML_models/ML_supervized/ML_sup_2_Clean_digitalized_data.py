...
def clean_data(raw_data_path : str, dest_path : str ):
    ...
    # Data Engineering
    data['conc_XX'] = data['Masse_XX'] / (data['Volume_YY'] + data['Volume_YY_ajout2'])
    data['volume_O2'] = data['Debit_air_synthetique'] * data['Temps_oxidation']
    data.drop(columns=['Masse_XX','Volume_YY','Volume_YY_ajout2','Debit_air_synthetique','Temps_oxidation'],inplace=True)
    ...
    data.drop(columns=['m_XY', 'm_YY', 'Abs_800'], inplace=True)

    # Export data and target
    target = data[['QC_Conc_XY', 'QC_Categorie']]
    target.to_csv(f'{dest_path}y.csv')
    data.drop(columns=['QC_Conc_XY', 'QC_Categorie'],inplace=True)
