...
@click.command()
@click.option(
    "--dataset", prompt='Dataset type (OGD/analyses):', default='OGD',
    help="Collect data, can be 'OGG' or 'analyses'")
@click.option(
    "--random_seed", default=42,
    help="Rand seed to split dataset")

def collect_data(dataset : str = 'OGD', split : float = 0.8, random_seed : int = 42):
    """
    ...
    if dataset == 'OGD':
        ...
        with open(f'data/{date}/OGD_metadata.pkl', 'wb') as file: 
            pickle.dump(metadata, file) 
            print(f'Dataset OGD saved in folder data/{date}')
