...
def preprocess_spectral(idx_train, idx_val, data_raman, data_uv, mode='pca'):
    ...
    data_raman_train = data_raman[data_raman.index.isin(idx_train)].sort_index().fillna(0)
    data_raman_val = data_raman[data_raman.index.isin(idx_val)].sort_index().fillna(0)
    
    data_uv_train = data_uv[data_uv.index.isin(idx_train)].sort_index().fillna(0)
    data_uv_val = data_uv[data_uv.index.isin(idx_val)].sort_index().fillna(0)
...
def run_data_prep(raw_data_path: str, dest_path: str, mode: str, split : float, save : str = False):
    ...
    with open(f'{raw_data_path}/XY_metadata.pkl', 'rb') as file: 
        df = pickle.load(file)
        df.loc[:,'Batch_XY_date'] = pd.to_datetime(df['Batch_XY_date'].copy())
        df.sort_values(by='Batch_XY_date',inplace=True)
    ...
    idx_train = df_train['Batch_XY_name']
    idx_val = df_val['Batch_XY_name']
