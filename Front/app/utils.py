...
def update_preds_from_UV(batch_name,conc):
    ...
    response = requests.post(f"http://fastapi-database:8000/Predictions/update/{id}",data=json.dumps(new_pred))
...
def update_MPs():
    ...
    batch_YY = MP.loc[MP.MP_nom=='YY',['MP_ref_fournisseur','MP_stock','MP_unite']].values
    return batch_C,batch_K,batch_YY
...
def predict_XY_concentration(data,heure_debut):
    ...
    response = requests.post(url = "http://fastapi-ml:8001/predict/elasticnet",
...
def fetch_MP():
    ...
    try:
        response = requests.get(f'http://fastapi-database:8000/matieres_premieres/')
        K_name,C_name,YY_name = get_matieres_premieres(response)
        default_batch = dict({'K':K_name,'C':C_name,'YY':YY_name})

        try:
            response = requests.get(f'http://fastapi-database:8000/XX/')
            XX_all = pd.DataFrame(response.json())
            # on filtre pour n'avoir que les batch qui sont termines et qui ont du stock
            XX_all = XX_all.loc[(~XX_all.Batch_XX_heure_fin.isnull()) and (XX_all.Batch_XX_Stock > 0)]
...
def update_stock_product(data):
        ...
        'Content-Type': 'application/json'}
        status=dict()
        response = requests.get(f'http://fastapi-database:8000/Produit/name/{data["Envoi_produit_batch"]}')
        ...
        if new_value >= 0:
            ...
            response = requests.post(f'http://fastapi-database:8000/Produit/update/{updated_batch["Batch_Produit_id"]}', headers=headers, data=json.dumps(updated_batch))
...
def update_stock_XY_additif(data):
        ...
        response = requests.get(f'http://fastapi-database:8000/XY/name/{data["Batch_Produit_XY_batch"]}')
        ...
        if new_value >= 0:
            ...
            response = requests.post(f'http://fastapi-database:8000/XY/update/{updated_batch["Batch_XY_id"]}', headers=headers, data=json.dumps(updated_batch))
            status['post_XY'] = response.status_code
        else :
            status['post_XY'] = None
        
        response = requests.get(f'http://fastapi-database:8000/matieres_premieres/name/{data["Batch_Produit_additif_batch"]}')
        ...
        if new_value >= 0:
            ...
            response = requests.post(f'http://fastapi-database:8000/matieres_premieres/update/{updated_batch["MP_id"]}', headers=headers, data=json.dumps(updated_batch))
...
def update_stocks_K_C(data):
        ...
        status=dict()
        # update stock of K 
        response = requests.get(f'http://fastapi-database:8000/matieres_premieres/name/{data["Batch_XX_K_batch"]}')
        ...
        # update stock of C
        response = requests.get(f'http://fastapi-database:8000/matieres_premieres/name/{data["Batch_XX_C_batch"]}')
        ...
        if (new_value_K >= 0) and (new_value_C >= 0):
            ...
            response = requests.post(f'http://fastapi-database:8000/matieres_premieres/update/{updated_batch_K["MP_id"]}', headers=headers, data=json.dumps(updated_batch_K))
            status['post_K'] = response.status_code
        
            updated_batch_C['MP_stock'] = new_value_C
            response = requests.post(f'http://fastapi-database:8000/matieres_premieres/update/{updated_batch_C["MP_id"]}', headers=headers, data=json.dumps(updated_batch_C))
...
def update_stocks_XX_YY(data):
        """ met à jour le stock de XX et YY après fabrication de XY """
        ...
        # update stock of XX 
        response = requests.get(f'http://fastapi-database:8000/XX/name/{data["Batch_XY_XX_batch"]}')
        status['get_XX'] = response.status_code
        updated_batch_XX = response.json()
        new_value_XX = float(updated_batch_XX['Batch_XX_Stock'] - float(data['Batch_XY_XX_masse']))
        
        # update stock of YY
        response = requests.get(f'http://fastapi-database:8000/matieres_premieres/name/{data["Batch_XY_YY_batch"]}')
        status['get_YY'] = response.status_code
        updated_batch_YY = response.json()
        new_value_YY = updated_batch_YY['MP_stock'] - int(data['Batch_XY_YY_Volume'])

        if (new_value_XX >= 0) and (new_value_YY >= 0):
            updated_batch_XX['Batch_XX_Stock'] = new_value_XX
            response2 = requests.post(f'http://fastapi-database:8000/XX/update/{updated_batch_XX["Batch_XX_id"]}', headers=headers, data=json.dumps(updated_batch_XX))
            status['post_XX'] = response2.status_code

            updated_batch_YY['MP_stock'] = new_value_YY
            response = requests.post(f'http://fastapi-database:8000/matieres_premieres/update/{updated_batch_YY["MP_id"]}', headers=headers, data=json.dumps(updated_batch_YY))
            status['post_YY'] = response.status_code
...
def get_last_10_batch():
    ...
    last_10_YY=data.loc[data.MP_nom == 'YY'].sort_values('MP_id')['MP_ref_fournisseur'].values[-10:][::-1]
    # get last XX
    ...
    return last_10_K, last_10_C, last_10_YY, last_10_XX
...
def get_matieres_premieres(response):
    ...
    try:
        ...
        YY_name=data.loc[data.MP_nom == 'YY'].sort_values('MP_id')['MP_ref_fournisseur'].values[-1]
    except:
        K_name=C_name=YY_name='na'
    return K_name,C_name,YY_name
