...
on fait le main et on inclu les router


make folder router and add db_n.py python file for each db table 

go to db_n.py and create fastapi route (it needs a pydantic display)

make shemas.py file at the root and create pydantic model to transfer info (passe plat)
    1 depuis l'user (formulaire) vers la db -- BatchBase 
    2 depuis la db vers l'interface user -- BatchDisplay

go to db_n.py and ceate the route (it needs connection to db)

create database folder : create db_connect.py to create connexion to database 
    set environ variable USER PASS NAME for the DB

go to db_n.py and ceate the route (it needs a function to make the call to the db)


create in database folder : the db_ogd_step1.py and write SQL logic in functions (CRUDE)
    it need the table definition

create in database folder : the models.py and define tables structures in class


go to db_n.py and ceate the variable requesting all the parameters 
